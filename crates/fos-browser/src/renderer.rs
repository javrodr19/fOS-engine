...
impl PageRenderer {
    ...
    fn paint(
        ...
        // Paint each layout box
        self.paint_tree(&mut canvas, layout_tree, document, styles);
        
        // Get pixels as RGBA bytes
        Some(canvas.as_rgba_bytes())
    }
    
    /// Paint layout tree recursively
    fn paint_tree(
        &self,
        canvas: &mut Canvas,
        layout_tree: &LayoutTree,
        ...
        if let Some(root_id) = layout_tree.root() {
            self.paint_box(canvas, layout_tree, root_id, document, styles);
        }
    }
            ...
            if let Some(node) = document.tree().get(node_id) {
                if let Some(text) = node.as_text() {
                    ...
                    self.paint_text(
                        ...
                        text,
                        dims.content.x,
                        dims.content.y + font_size, // Baseline offset
                        text_color,
                        font_size,
                    );
                }
    ...
    /// Simple text painting using pixel drawing
    fn paint_text(
        &self,
        canvas: &mut Canvas,
        text: &str,
        ...
        // Very simple text rendering - just draw characters as blocks
        // In production, would use proper font rasterization
        let char_width = (font_size * 0.6) as f32;
        ...
        for c in text.chars() {
            ...
            // This is obviously not real text rendering, but demonstrates the pipeline
            ...
            for (row, &bits) in pattern.iter().enumerate() {
                for col in 0..8 {
                    if (bits >> (7 - col)) & 1 == 1 {
                        let px = (x_pos + col as f32 * scale) as u32;
                        let py = (y - font_size + row as f32 * scale) as u32;
